    ...
    <a-spin :spinning="loading">
      <a-form :form="form">
        ...
        <a-form-item label="电阻 " :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['resistanceStr', { rules: [{ required: true, message: '必填' }] }]" />
        </a-form-item>
        <a-form-item label="电流 " :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['current', { rules: [{ required: true, message: '必填' }] }]" />
        </a-form-item>
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.$nextTick(() => {
          ...
          this.$http.post('/DataManage/Battery/GetTheData', { id: id }).then(resJson => {
    ...
    handleSubmit() {
      this.form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          this.loading = true
          this.$http.post('/DataManage/Battery/SaveData', this.entity).then(resJson => {
            this.loading = false
