  ...
  >
    <a-spin :spinning="loading">
      <a-form :form="form">
        ...
        <a-form-item label="开启关闭" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['onOff', { rules: [{ required: true, message: '必填' }] }]" />
        </a-form-item>
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.$nextTick(() => {
          ...
          this.$http.post('/DataManage/A5NodeOnOff/GetTheData', { id: id }).then(resJson => {
    ...
    handleSubmit() {
      this.form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          this.loading = true
          this.$http.post('/DataManage/A5NodeOnOff/SaveData', this.entity).then(resJson => {
            this.loading = false
