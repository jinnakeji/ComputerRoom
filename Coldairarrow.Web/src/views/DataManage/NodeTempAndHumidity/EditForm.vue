  ...
  >
    <a-spin :spinning="loading">
      <a-form :form="form">
        ...
        <a-form-item label="正常温度" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['temperature', { rules: [{ required: true, message: '必填' }] }]" />
        </a-form-item>
        <a-form-item label="正常湿度" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['humidity', { rules: [{ required: true, message: '必填' }] }]" />
        </a-form-item>
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.$nextTick(() => {
          ...
          this.$http.post('/DataManage/NodeTempAndHumidity/GetTheData', { id: id }).then(resJson => {
    ...
    handleSubmit() {
      this.form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          this.loading = true
          this.$http.post('/DataManage/NodeTempAndHumidity/SaveData', this.entity).then(resJson => {
            this.loading = false
