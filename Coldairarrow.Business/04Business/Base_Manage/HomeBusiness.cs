...
namespace Coldairarrow.Business.Base_Manage
{
    ...
    {
        public AjaxResult SubmitLogin(string userName, string password, bool isMd5 = false)
        {
            if (userName.IsNullOrEmpty() || password.IsNullOrEmpty())
                return Error("账号或密码不能为空！");
            password = isMd5 ? password : password.ToMD5String();
        ...
        public AjaxResult CheckUser(string userName, string password)
        {
            if (userName.IsNullOrEmpty() || password.IsNullOrEmpty())
                return Error("账号或密码不能为空！");
            ...
            if (theUser != null)
            {
                return Success<object>(new { UserId = theUser.Id });
            }
            else
                return Error("账号或密码不正确！");
        }
